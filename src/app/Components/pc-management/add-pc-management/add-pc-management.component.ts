...
@Component({
  selector: 'app-add-pc-management',
  templateUrl: './add-pc-management.component.html',
  styleUrls: ['./add-pc-management.component.css']
})
export class AddPcManagementComponent implements OnInit {

  showAddPCPopup: boolean;
  pc: PCListDTO;
  ...
  constructor(private formBuilder: FormBuilder, private partService: PartService, private pcService: PCService, private toastr: ToastrService) {
    ...
    //this.selectedProcessor = 0;
    //this.selectedMotherboard = 0;
    //this.selectedRam = 0;
    //this.selectedGraphicard = 0;
    //this.selectedPowerSupply = 0;
    //this.selectedCasing = 0;
...
// yahn se copy paste h km
  ...
  onSubmitForm() {
    ...
    formData.append('image1', this.image1.nativeElement.files[0]);
    formData.append('image2', this.image2.nativeElement.files[0]);
    formData.append('image3', this.image3.nativeElement.files[0]);
    formData.append('image4', this.image4.nativeElement.files[0]);

    this.pcService.PostPCDetails("PC", "PostPCDetails", formData).subscribe(
      (data: any) => {
        if (data) {
          this.onReset();
          this.toastr.success('PC', 'PC added successfully.');
          this.CloseAddPCPopup();
        } else {
